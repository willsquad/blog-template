<?php include('include/header.php');?>

            <!-- Body Content -->
            <div class="col-12 body_content max_width article_body">
                <!-- ROW -->
                <div class="row">
                    <!-- LHS -->
                    <div class="col-12 col-md-7 col-lg-8 body_content__lhs">
                        <!-- Article container -->
                        <div class="body_content__lhs__container">
                            <!-- Article Large -->
                            <div class="article article_large">
                                <img src="files/images/article_large.jpeg" alt="">
                                
                                <div class="article_details">
                                    <h6 class="article_category"><a href="#">Category</a> <span class="details_divider">/</span> <span class="details_date">September 14, 2018</span></h6>
                                    <h3 class="article_large_heading">Lorem ipsum dolor sit amet consectetur, adipisicing elit. Assumenda, dignissimos.</h3>
                                   <!--  <div class="author_div"><a href="#">
                                        <img src="files/images/profile.png" alt="">John Doe
                                        </a>
                                    </div> -->
                                    <p class="article_large_summary">Lorem ipsum dolor sit amet, consectetur adipiscing elit,sed do eiusmod tempor incididunt ut labore et dolore magnaaliqua. Ut enim ad minim veniam, quis nostrud exercitationullamco laboris nisi ut aliquip ex ea commodo consequat. Lorem ipsum dolor sit amet, consectetur adipiscing elit,sed do eiusmod tempor incididunt ut labore et dolore magnaaliqua. Ut enim ad minim veniam, quis nostrud exercitationullamco laboris nisi ut aliquip ex ea commodo consequat.</p>

                                    <div class="article_media">
                                        <img src="files/images/coast.jpg" alt="">
                                    </div>
                                    
                                    <p class="article_large_summary">Lorem ipsum dolor sit amet, consectetur adipiscing elit,sed do eiusmod tempor incididunt ut labore et dolore magnaaliqua. Ut enim ad minim veniam, quis nostrud exercitationullamco laboris nisi ut aliquip ex ea commodo consequat.</p>

                                    <div class="article_media">
                                        <div class="video_container">
                                            <iframe src="https://www.youtube.com/embed/4xls7K_xFBQ?showinfo=0&modestbranding=0" frameborder="0"  allowfullscreen></iframe>
                                        </div>
                                    </div>

                                    <p class="article_large_summary">Lorem ipsum dolor sit amet, consectetur adipiscing elit,sed do eiusmod tempor incididunt ut labore et dolore magnaaliqua. Ut enim ad minim veniam, quis nostrud exercitationullamco laboris nisi ut aliquip ex ea commodo consequat. Lorem ipsum dolor sit amet, consectetur adipiscing elit,sed do eiusmod tempor incididunt ut labore et dolore magnaaliqua. Ut enim ad minim veniam, quis nostrud exercitationullamco laboris nisi ut aliquip ex ea commodo consequat.</p>

                                     <div class="article_media">
                                        <img src="files/images/featured2.jpeg" alt="">
                                    </div>
                                    
                                    <p class="article_large_summary">Lorem ipsum dolor sit amet, consectetur adipiscing elit,sed do eiusmod tempor incididunt ut labore et dolore magnaaliqua. Ut enim ad minim veniam, quis nostrud exercitationullamco laboris nisi ut aliquip ex ea commodo consequat.</p>

                                    <div class="article_details__bottom">
                                        <div class="article_details__bottom__lhs">
                                            <i class="fas fa-tags"></i>
                                            <a href="#" class="article_tags">Tag 1</a>
                                            <a href="#" class="article_tags">Tag 2</a>
                                            <a href="#" class="article_tags">Tag 3</a>
                                        </div>
                                        <div class="article_details__bottom__rhs">
                                            <div class="article_like_share">
                                                <div class="likes_div">
                                                    <i class="far fa-heart"></i>
                                                    125
                                                </div>         
                                            </div>
                                            <div class="article_social_icons">
                                                <i class="fab fa-facebook-f"></i>
                                                <i class="fab fa-twitter"></i>
                                                <i class="fab fa-google-plus-g"></i>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <!-- Article Large -->

                            <!-- Author Details -->
                            <!-- <div class="author_details">
                                <div class="author_img">
                                    <img src="files/images/profile.png" alt="">
                                </div>
                                <div class="author_text">
                                    <h5 class="author_text_title">John Doe</h5>
                                    <p class="author_text_summary">Lorem ipsum dolor sit amet, consectetur adipisicing elit. Perferendis unde facere quam, repudiandae recusandae possimus!</p>
                                    <div class="article_social_icons">
                                        <i class="fab fa-facebook-f"></i>
                                        <i class="fab fa-twitter"></i>
                                        <i class="fab fa-google-plus-g"></i>
                                    </div>
                                </div>
                            </div> -->
                            <!-- Author Details -->

                            <!-- Comments -->
                            <!-- <div class="col-12 comments_div">
                                <h4 class="comments_title">Comments</h4>
                            </div> -->
                            <!-- Comments -->
                        </div>
                        <!-- Article container -->
                    </div>
                    <!-- LHS -->

                    <!-- RHS -->
                    <?php include('include/sidebar.php'); ?>
                    <!-- RHS -->
                </div>
                <!-- ROW -->
                
                <!-- Related Articles Container -->
                <div class="row article_small_container related_articles">

                    <div class="col-12 related_articles_header">
                        <div class="prev_related_article">
                            <i class="far fa-arrow-alt-circle-left"></i>
                        </div>
                        <h4 class="related_articles_title">Related Articles</h4>
                        <div class="next_related_article">
                            <i class="far fa-arrow-alt-circle-right"></i>
                        </div>
                    </div>

                    <!-- Related Articles Slider Container -->
                    <div id="related_articles_slider_container" class="">

                        <div class="col-12 col-lg-4">
                            <!-- Article Small -->
                            <div class="article article_small">
                                <img src="files/images/coast.jpg" alt="">

                                <div class="article_details">
                                    <h6 class="article_category"><a href="#">Category</a></h6>
                                    <h3 class="article_large_heading"><a href="article.php">Ipsm sit amet consectetur adipisicing elit</a></h3>

                                    <div class="article_details__bottom">
                                        <div class="article_details__bottom__lhs">
                                            <a href="article.php">Read More</a>
                                        </div>
                                        <div class="article_details__bottom__rhs">
                                            <i class="far fa-heart"></i> 110
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <!-- Article Small -->
                        </div>
                        <div class="col-12 col-lg-4">
                            <!-- Article Small -->
                            <div class="article article_small">
                                <img src="files/images/scotland.jpg" alt="">
                            
                                <div class="article_details">
                                    <h6 class="article_category"><a href="#">Category</a></h6>
                                    <h3 class="article_large_heading"><a href="article.php">Dolor sit amet consectetur adipisicing elit</a></h3>


                                    <div class="article_details__bottom">
                                        <div class="article_details__bottom__lhs">
                                            <a href="article.php">Read More</a>
                                        </div>
                                        <div class="article_details__bottom__rhs">
                                            <i class="far fa-heart"></i> 85
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <!-- Article Small -->
                        </div>

                        <div class="col-12 col-lg-4">
                            <!-- Article Small -->
                            <div class="article article_small">
                                <img src="files/images/featured2.jpeg" alt="">

                                <div class="article_details">
                                    <h6 class="article_category"><a href="#">Category</a></h6>
                                    <h3 class="article_large_heading"><a href="article.php">Ipsm sit amet consectetur adipisicing elit</a></h3>

                                    <div class="article_details__bottom">
                                        <div class="article_details__bottom__lhs">
                                            <a href="article.php">Read More</a>
                                        </div>
                                        <div class="article_details__bottom__rhs">
                                            <i class="far fa-heart"></i> 110
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <!-- Article Small -->
                        </div>


                        <div class="col-12 col-lg-4">
                            <!-- Article Small -->
                            <div class="article article_small">
                                <img src="files/images/coast.jpg" alt="">

                                <div class="article_details">
                                    <h6 class="article_category"><a href="#">Category</a></h6>
                                    <h3 class="article_large_heading"><a href="article.php">Ipsm sit amet consectetur adipisicing elit</a></h3>

                                    <div class="article_details__bottom">
                                        <div class="article_details__bottom__lhs">
                                            <a href="article.php">Read More</a>
                                        </div>
                                        <div class="article_details__bottom__rhs">
                                            <i class="far fa-heart"></i> 110
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <!-- Article Small -->
                        </div>
                        <div class="col-12 col-lg-4">
                            <!-- Article Small -->
                            <div class="article article_small">
                                <img src="files/images/scotland.jpg" alt="">
                            
                                <div class="article_details">
                                    <h6 class="article_category"><a href="#">Category</a></h6>
                                    <h3 class="article_large_heading"><a href="article.php">Dolor sit amet consectetur adipisicing elit</a></h3>


                                    <div class="article_details__bottom">
                                        <div class="article_details__bottom__lhs">
                                            <a href="article.php">Read More</a>
                                        </div>
                                        <div class="article_details__bottom__rhs">
                                            <i class="far fa-heart"></i> 85
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <!-- Article Small -->
                        </div>

                        <div class="col-12 col-lg-4">
                            <!-- Article Small -->
                            <div class="article article_small">
                                <img src="files/images/featured2.jpeg" alt="">

                                <div class="article_details">
                                    <h6 class="article_category"><a href="#">Category</a></h6>
                                    <h3 class="article_large_heading"><a href="article.php">Ipsm sit amet consectetur adipisicing elit</a></h3>

                                    <div class="article_details__bottom">
                                        <div class="article_details__bottom__lhs">
                                            <a href="article.php">Read More</a>
                                        </div>
                                        <div class="article_details__bottom__rhs">
                                            <i class="far fa-heart"></i> 110
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <!-- Article Small -->
                        </div>

                    </div>
                    <!-- #Related articles slider container -->


                </div>
                <!-- Related Articles main Container -->

            </div>
            <!-- Body Content -->

<?php include('include/footer.php');?>